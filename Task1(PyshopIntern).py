...
def get_score(game_stamps, offset):
    ...
    if offset <= game_stamps[-1].get('offset'):
        index = 0
        while game_stamps[index].get('offset') < offset:
            index += 1
        if game_stamps[index].get('offset') == offset:
            return game_stamps[index].get('score').get('home'), \
                   game_stamps[index].get('score').get('away')


score = get_score(game_stamps, offset=100000)
